"""
검색 부하테스트용 상품 데이터 적재 스크립트 (Bulk Insert 최적화 적용)
"""
  ...
  while total_products < target_products:
    ...
    if product_rows:
        psycopg2.extras.execute_values(
            cur,
            """
            INSERT INTO products (category_id, store_id, name, description, option_type, status, created_at, updated_at)
            VALUES %s
            """,
            product_rows,
            template="(%s, %s, %s, %s, %s, %s, NOW() - (random() * INTERVAL '365 days'), NOW() - (random() * INTERVAL '30 days'))",
            page_size=1000
        )
    ...
    if item_insert_rows:
        psycopg2.extras.execute_values(
            ...
            (product_id, sku, price, main_image_url, status, display_option_name, is_default, created_at, updated_at)
            VALUES %s
            """,
            item_insert_rows,
            template="(%s, %s, %s, %s, %s, %s, %s, NOW() - (random() * INTERVAL '365 days'), NOW() - (random() * INTERVAL '30 days'))",
            page_size=1000
        )
